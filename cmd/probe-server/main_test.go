...
func TestLoadConfigRejectsOverlappingPrivateDirectories(t *testing.T) {
	values := map[string]string{
		...
		"PROBE_AGENT_CA_DIRECTORY":        "/var/lib/shared",
...
func TestLoadConfigUsesPrivateRuntimeDefaults(t *testing.T) {
	...
	if config.AgentCADirectory == config.DataDirectory || config.AgentCAPublicDirectory == config.DataDirectory || config.AgentCADirectory == config.AgentCAPublicDirectory {
